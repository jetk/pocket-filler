...
Mode button cycles **Draw → Fill → Move**. Separate **♪ Dance** toggle. See
README.md for the user-facing controls.
...
src/dance.js    choreography (which nodes step where). Pure.
...
- **Don't `save()` during dance.** The snap-back would disagree with what's on
  disk. Dance ticks mutate and redraw only.
...
23 tests, all in node with `assert` — no framework, no fixtures. The geometry,
codec and choreography are covered; rendering and input are not. Non-trivial
...
`window.pf` exposes `state`, `moveNode(from, to)`, `redraw()`, `faces()`.
`moveNode` is the seam animation should use — the node drag is already just
...
- **Move a whole shape.** Not built. Confirmed feasible with no schema change:
  union-find over shared node positions yields connected components (the sample
  robot drawing splits into 11). Caveat: lines that merely cross are *not*
  connected, since intersections live only inside `planar.js`; and shapes
  touching at one node merge into one component. Selection UX undecided —
  long-press in Move mode, or a fourth mode.
